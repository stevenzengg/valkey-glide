    ...
    C: ConnectionLike + Connect + Clone + Send + Sync + 'static,
{
    ...
    /// 1. Reverse IP lookup via slot map (for raw IPs returned by MOVED/ASK)
    /// 2. DNS/address-resolver → direct connection lookup
    ...
    pub(crate) fn resolve_address_with_path(&self, address: &str) -> (String, &'static str) {
        ...
        let params = self.cluster_params.read();
                ...
                if now_secs >= last_health + interval {
                    ...
                    if in_recovery {
                        log_info_lazy!(
                            "cluster",
                            format!(
                                "poll_flush health (recovery): in_flight_requests={}",
                                in_flight
                            )
                        );
                    } else {
                        log_debug_lazy!(
                            "cluster",
                            format!(
                                "poll_flush health (healthy): in_flight_requests={}",
                                in_flight
                            )
                        );
                    }
